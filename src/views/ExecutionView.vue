  ...
  <div class="page-shell space-y-6">
    <header class="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <h1 class="page-title mt-2">Ejecución de References Codex</h1>
        <p class="page-description">El navegador consulta <code>SESSION_STATE.json</code> mediante FLOWS. Puedes abandonar esta página y recuperar la sesión desde Inicio.</p>
      </div>
    ...
    <template v-else-if="status && session">
      ...
      <div v-if="session.stopped" class="warning-banner"><strong>Ejecución detenida.</strong> No se lanzarán nuevos batches. Las tareas ya completadas permanecen en el workspace.</div>
      ...
      <section class="surface-card">
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 class="mt-4 text-2xl font-black text-slate-950 dark:text-white">{{ session.opportunityTitle || session.project || 'Generación de referencias' }}</h2>
      ...
      <section class="grid gap-4 lg:grid-cols-3">
        <article class="surface-card-flat p-5 lg:col-span-2">
          ...
          <dl class="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <div><dt class="text-xs font-bold uppercase tracking-wide text-slate-400">Unidad</dt><dd class="mt-1 text-sm font-bold text-slate-800 dark:text-slate-100">{{ session.unit || '—' }}</dd></div>
            <div><dt class="text-xs font-bold uppercase tracking-wide text-slate-400">Proyecto</dt><dd class="mt-1 text-sm font-bold text-slate-800 dark:text-slate-100">{{ session.project || '—' }}</dd></div>
